  ...
  >
    ...
    >
      ...
      <UiCard>
        <UiCardContent class="grid gap-1.5">
          <p class="text-sm font-medium text-muted-foreground">
            {{ $t("analytics.ai.stats.hours-remaining") }}
          </p>
          <p class="text-3xl font-bold">
            {{ format((ai.remaining.minutes ?? 0) / 60) }} h
          </p>
        </UiCardContent>
      </UiCard>
      <UiCard>
        <UiCardContent class="grid gap-1.5">
          <p class="text-sm font-medium text-muted-foreground">
            {{ $t("analytics.ai.stats.hours-available") }}
          </p>
          <p class="text-3xl font-bold">
            {{ format((ai.available.minutes ?? 0) / 60) }} h
          </p>
        </UiCardContent>
      </UiCard>
    </header>
    ...
    <UiCard>
      <UiCardHeader>
        ...
        <UiCardDescription>{{ $t("analytics.ai.graph.video", { count: separate(totalTokens) }) }}</UiCardDescription>
      </UiCardHeader>
      <UiCardContent>
        <AreaChart
          v-if="ai"
